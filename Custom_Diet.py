...
def get_data(json_file):
    ...
    for recipies in json_file:
        dataset["Name"].append(recipies["Name"])
        dataset["RecipeIngredientParts"].append(recipies["RecipeIngredientParts"])
        dataset["Calories"].append(recipies["Calories"])
        dataset["RecipeInstructions"].append(recipies["RecipeInstructions"])
...
def display_recommendation(dataset):
    ...
    with st.spinner('Generating recommendations...'):
        ...
        for _, row in dataset.iterrows():
            ...
            expander.markdown(f"- {ingredients}")
            expander.markdown('<h5 style="text-align: center;font-family:sans-serif;">Recipe Instructions:</h5>', unsafe_allow_html=True)
            expander.markdown(f"- {instructions}")
            expander.markdown('<h5 style="text-align: center;font-family:sans-serif;">Total Calories Intake:</h5>', unsafe_allow_html=True)
            expander.markdown(f"Total Calories Intake: {calories}")
...
def test_charts(files):
    ...
    st.markdown(f"""<h3 style="color:#7FFF00;">Basic Statistical Summary</h3>""", unsafe_allow_html=True)
